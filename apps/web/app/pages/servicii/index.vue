  ...
  <div>
    <BentoCard class="!mb-6">
      <div class="text-center max-w-2xl mx-auto">
        <p class="text-[11px] font-bold text-primary uppercase tracking-wider mb-3">Servicii</p>
        <h1 class="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white">
          Tot ce-ți trebuie pentru un IT fără griji
        </h1>
        <p class="text-slate-500 dark:text-slate-400 mt-3">
          De la o curățare simplă de laptop până la administrarea infrastructurii întregi.
        </p>
      </div>
    </BentoCard>

    <BentoGrid class="grid gap-6 md:grid-cols-2 lg:grid-cols-3 auto-rows-fr [column-count:unset]">
      <BentoCard v-for="s in services" :key="s.slug" class="flex h-full min-h-[22rem] flex-col">
        ...
        <div class="flex items-center justify-between mt-auto pt-4 border-t border-slate-100 dark:border-dark-border">
          <div class="text-sm font-bold text-primary">De la {{ s.priceFrom }} {{ s.currency }}</div>
          <UiButton :to="`/servicii/${s.slug}`" variant="outline" size="sm">
            Detalii <Icon name="fa6-solid:arrow-right" />
          </UiButton>
        </div>
      </BentoCard>
    </BentoGrid>
